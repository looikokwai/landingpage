...
export default function Edit({ marqueeText }) {
  const { t } = useTranslation()
  const theme = useTheme()
  const { data, setData, put, processing, errors } = useForm({
    text: marqueeText.text || '',
    is_active: marqueeText.is_active ? 'true' : 'false',
    order: marqueeText.order || 0
  })

  const submit = e => {
    e.preventDefault()
    put(route('admin.marquee-texts.update', marqueeText.id))
  }

  return (
    <>
      <Head title={t('marquee_text.edit')} />
      <Box sx={{ py: 4 }}>
