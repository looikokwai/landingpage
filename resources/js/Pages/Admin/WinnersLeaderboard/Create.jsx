...
export default function Create({ categories }) {
  ...
  const submit = e => {
    e.preventDefault()
    post(route('admin.winners-leaderboard.store'), {
      onSuccess: () => reset(),
    })
  }
  ...
  const categoryOptions = Object.entries(categories).map(([value, label]) => ({
    value,
    label: t(`winner.category_options.${label.toLowerCase().replace(' ', '_')}`) // Assuming labels are like 'Latest Winner'
  }))

  return (
    <>
      <Head title={t('winner.create')} />
      <Box sx={{ py: 4 }}>
