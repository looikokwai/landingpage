...
export default function Create({ statusOptions }) {
  ...
  const submit = e => {
    ...
    router.post(route('admin.special-tasks.store'), formData, {
      onSuccess: () => reset('photo'),
      onError: (errors) => {
        console.error('SpecialTasks Create.jsx - Submission errors:', errors);
      },
    });
  }
  ...
  return (
    <>
      <Head title={t('special_task.create')} />
      <Box sx={{ py: 4 }}>
